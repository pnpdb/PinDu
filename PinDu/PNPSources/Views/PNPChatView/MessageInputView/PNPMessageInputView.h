...
#import "PNPMessageTextView.h"

typedef NS_ENUM(NSInteger, PNPMessageInputViewStyle) {
    // 分两种,一种是iOS6样式的，一种是iOS7样式的
    PNPMessageInputViewStyleQuasiphysical,
    PNPMessageInputViewStyleFlat
};

@protocol PNPMessageInputViewDelegate <NSObject>
...
@interface PNPMessageInputView : UIImageView

@property (nonatomic, weak) id <PNPMessageInputViewDelegate> delegate;

/**
 *  用于输入文本消息的输入框
 */
@property (nonatomic, weak, readonly) PNPMessageTextView *inputTextView;

/**
 *  当前输入工具条的样式
 */
@property (nonatomic, assign) PNPMessageInputViewStyle messageInputViewStyle;  // default is XHMessageInputViewStyleFlat
