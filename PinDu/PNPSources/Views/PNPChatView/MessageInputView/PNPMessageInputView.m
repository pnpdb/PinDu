...
#define kPNPTouchToRecord         @"按住 说话"
#define kPNPTouchToFinish         @"松开 结束"

@interface PNPMessageInputView ()<UITextViewDelegate>

@property (nonatomic, weak, readwrite) PNPMessageTextView *inputTextView;
 ...
 *  @param style 输入框样式类型
 */
- (void)setupMessageInputViewBarWithStyle:(PNPMessageInputViewStyle)style ;
...
@implementation PNPMessageInputView
...
- (UIButton *)createButtonWithImage:(UIImage *)image HLImage:(UIImage *)hlImage {
    UIButton *button = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, [PNPMessageInputView textViewLineHeight], [PNPMessageInputView textViewLineHeight])];
    if (image)
...
- (void)setupMessageInputViewBarWithStyle:(PNPMessageInputViewStyle)style {
    ...
    // 输入框
    CGFloat textViewLeftMargin = ((style == PNPMessageInputViewStyleFlat) ? 6.0 : 4.0);
    ...
    CGFloat height = [PNPMessageInputView textViewLineHeight];
    
    // 初始化输入框
    PNPMessageTextView *textView = [[PNPMessageTextView  alloc] initWithFrame:CGRectZero];
    ...
    // 配置不同iOS SDK版本的样式
    switch (style) {
        case PNPMessageInputViewStyleQuasiphysical: {
        ...
        case PNPMessageInputViewStyleFlat: {
    ...
    if (self.allowsSendVoice) {
        ...
        [button setTitle:kPNPTouchToRecord forState:UIControlStateNormal];
        [button setTitle:kPNPTouchToFinish forState:UIControlStateHighlighted];
...
- (void)setup {
    ...
    _messageInputViewStyle = PNPMessageInputViewStyleFlat;
}
...
+ (CGFloat)maxHeight {
    return ([PNPMessageInputView maxLines] + 1.0f) * [PNPMessageInputView textViewLineHeight];
}
