...
#import "PNPShareMenuView.h"
#import "PNPMacro.h"

// 每行有4个
#define kPNPShareMenuPerRowItemCount (kIsiPad ? 10 : 4)
#define kPNPShareMenuPerColum 2

@interface PNPShareMenuItemView : UIView
...
@implementation PNPShareMenuItemView

- (void)setup {
    if (!_shareMenuItemButton) {
        UIButton *shareMenuItemButton = [UIButton buttonWithType:UIButtonTypeCustom];
        shareMenuItemButton.frame = CGRectMake(0, 0, kPNPShareMenuItemWidth, kPNPShareMenuItemWidth);
    ...
    if (!_shareMenuItemTitleLabel) {
        UILabel *shareMenuItemTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.shareMenuItemButton.frame), kPNPShareMenuItemWidth, KPNPShareMenuItemHeight - kPNPShareMenuItemWidth)];
...
@interface PNPShareMenuView () <UIScrollViewDelegate>
...
@implementation PNPShareMenuView
...
- (void)reloadData {
    ...
    CGFloat paddingX = 16;
    CGFloat paddingY = 10;
    for (PNPShareMenuItem *shareMenuItem in self.shareMenuItems) {
        NSInteger index = [self.shareMenuItems indexOfObject:shareMenuItem];
        NSInteger page = index / (kPNPShareMenuPerRowItemCount * kPNPShareMenuPerColum);
        CGRect shareMenuItemViewFrame = [self getFrameWithPerRowItemCount:kPNPShareMenuPerRowItemCount
                                                        perColumItemCount:kPNPShareMenuPerColum
                                                                itemWidth:kPNPShareMenuItemWidth
                                                               itemHeight:KPNPShareMenuItemHeight
                                                                 paddingX:paddingX
                                                                 paddingY:paddingY
                                                                  atIndex:index
                                                                   onPage:page];
        PNPShareMenuItemView *shareMenuItemView = [[PNPShareMenuItemView alloc] initWithFrame:shareMenuItemViewFrame];
    ...
    self.shareMenuPageControl.numberOfPages = (self.shareMenuItems.count / (kPNPShareMenuPerRowItemCount * 2) + (self.shareMenuItems.count % (kPNPShareMenuPerRowItemCount * 2) ? 1 : 0));
    [self.shareMenuScrollView setContentSize:CGSizeMake(((self.shareMenuItems.count / (kPNPShareMenuPerRowItemCount * 2) + (self.shareMenuItems.count % (kPNPShareMenuPerRowItemCount * 2) ? 1 : 0)) * CGRectGetWidth(self.bounds)), CGRectGetHeight(self.shareMenuScrollView.bounds))];
}
...
- (void)setup {
    ...
    if (!_shareMenuScrollView) {
        UIScrollView *shareMenuScrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - kPNPShareMenuPageControlHeight)];
    ...
    if (!_shareMenuPageControl) {
        UIPageControl *shareMenuPageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(self.shareMenuScrollView.frame), CGRectGetWidth(self.bounds), kPNPShareMenuPageControlHeight)];
