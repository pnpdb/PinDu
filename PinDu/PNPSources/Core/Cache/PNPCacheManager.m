...
@implementation PNPCacheManager

+ (instancetype)shareCacheManager {
    return self.manager;
}

+ (PNPCacheManager *)cacheManagerWithIdentifier:(NSString *)identifier {
    return [[PNPCacheManager alloc] initWithIdentifier:identifier];
}
...
+ (PNPCacheManager *)manager {
    static PNPCacheManager *sharedInstance = nil;
    static dispatch_once_t  onceToken;
    
    dispatch_once(&onceToken, ^{
        sharedInstance = [[PNPCacheManager alloc] initWithIdentifier:NSStringFromClass(self)];
    });
    ...
    for(int i = 0; i < 16; i ++) {
        for(int j = 0; j < 16; j++) {
            ...
            for(id name in list) {
                ...
                if(attr) {
                    [files addObject:[[PNPFileAttribute alloc] initWithPath:filePath attributes:attr]];
                }
            }
        }
    }
