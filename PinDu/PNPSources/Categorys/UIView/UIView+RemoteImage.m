...
#import "PNPCacheManager.h"

const char* const kPNPURLPropertyKey   = "PNPURLDownloadURLPropertyKey";
const char* const kPNPLoadingStateKey  = "PNPURLDownloadLoadingStateKey";
const char* const kPNPLoadingViewKey   = "PNPURLDownloadLoadingViewKey";

const char* const kPNPActivityIndicatorViewKey   = "PNPActivityIndicatorViewKey";

const char* const kPNPMessageAvatorTypeKey   = "PNPMessageAvatorTypeKey";
...
- (void)setActivityIndicatorView:(UIActivityIndicatorView *)activityIndicatorView {
    objc_setAssociatedObject(self, kPNPActivityIndicatorViewKey, activityIndicatorView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIActivityIndicatorView *)activityIndicatorView {
    return objc_getAssociatedObject(self, kPNPActivityIndicatorViewKey);
}

- (void)setMessageAvatorType:(PNPMessageAvatorType)messageAvatorType {
    objc_setAssociatedObject(self, &kPNPMessageAvatorTypeKey, [NSNumber numberWithInteger:messageAvatorType], OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (PNPMessageAvatorType)messageAvatorType {
    return (PNPMessageAvatorType)([objc_getAssociatedObject(self, &kPNPMessageAvatorTypeKey) integerValue]);
}

- (NSURL*)url {
    return objc_getAssociatedObject(self, kPNPURLPropertyKey);
}
...
- (void)setImageUrl:(NSURL *)url autoLoading:(BOOL)autoLoading {
    if(![url isEqual:self.url]) {
        objc_setAssociatedObject(self, kPNPURLPropertyKey, url, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (UIImageViewURLDownloadState)loadingState {
    return (NSUInteger)([objc_getAssociatedObject(self, kPNPLoadingStateKey) integerValue]);
}

- (void)setLoadingState:(UIImageViewURLDownloadState)loadingState {
    objc_setAssociatedObject(self, kPNPLoadingStateKey, @(loadingState), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (UIView *)loadingView {
    return objc_getAssociatedObject(self, kPNPLoadingViewKey);
}

- (void)setLoadingView:(UIView *)loadingView {
    [self.loadingView removeFromSuperview];
    
    objc_setAssociatedObject(self, kPNPLoadingViewKey, loadingView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)loadWithCompletionBlock:(void(^)(UIImage *image, NSURL *url, NSError *error))handler {
    ...
    dispatch_async(self.cachingQueue, ^{
        UIImage *cacheImage = [PNPCacheManager imageWithURL:weakSelf.url storeMemoryCache:YES];
        if (self.messageAvatorType != PNPMessageAvatorTypeNormal) {
            cacheImage = [PNPMessageAvatorFactory avatarImageNamed:cacheImage messageAvatorType:self.messageAvatorType];
        }
        if (cacheImage) {
...
- (void)cachingImageData:(NSData *)imageData url:(NSURL *)url {
    dispatch_async(self.cachingQueue, ^{
        if (imageData) {
            [PNPCacheManager storeData:imageData forURL:url storeMemoryCache:NO];
            UIImage *image = [UIImage imageWithData:imageData];
            if (image)
                [PNPCacheManager storeMemoryCacheWithImage:image forURL:url];
        }
    });
}
...
- (UIImage *)didFinishDownloadWithData:(NSData *)data forURL:(NSURL *)url error:(NSError *)error {
    ...
    if (self.messageAvatorType != PNPMessageAvatorTypeNormal) {
        image = [PNPMessageAvatorFactory avatarImageNamed:image messageAvatorType:self.messageAvatorType];
    }
    if([url isEqual:self.url]) {
        if(error) {
