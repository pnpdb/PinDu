...
@implementation PNPAudioPlayerHelper
    ...
    if (fileName.length > 0) {
        ...
        if (_playingFileName && [fileName isEqualToString:_playingFileName]) {//上次播放的录音
            ...
            /*
             ...
             AVAudioPlayer *pl = [[AVAudioPlayer alloc] initWithContentsOfURL:[NSURL fileURLWithPath:[PNPVoiceCommonHelper getPathByFileName:fileName ofType:@"wav"]] error:nil];
             */
...
- (void)setDelegate:(id<PNPAudioPlayerHelperDelegate>)delegate {
...
+ (id)shareInstance {
    static PNPAudioPlayerHelper *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[PNPAudioPlayerHelper alloc] init];
    });
    return instance;
}
