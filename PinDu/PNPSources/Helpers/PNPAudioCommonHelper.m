...
#import "PNPAudioCommonHelper.h"
#import "PNPMacro.h"

@implementation PNPAudioCommonHelper
...
+ (void)removeRecordedFileWithOnlyName:(NSString*)fileName block:(DidDeleteAudioFileBlock)block {
    ...
    [PNPAudioCommonHelper removeRecordedFile:fileName type:@"amr"];
    
    NSString *originWavFile = [fileName stringByReplacingOccurrencesOfString:@"wavToAmr" withString:@""];
    [PNPAudioCommonHelper removeRecordedFile:originWavFile type:@"wav"];
    
    NSString *convertedWavFile = [originWavFile stringByAppendingString:@"amrToWav"];
    [PNPAudioCommonHelper removeRecordedFile:convertedWavFile type:@"amr"];
...
+ (void)removeAudioFile:(NSString*)exception block:(DidDeleteAudioFileBlock)block {
    NSString *path = [PNPAudioCommonHelper getCacheDirectory];
...
{
    NSString* fileDirectory = [[[PNPAudioCommonHelper getCacheDirectory] stringByAppendingPathComponent:_fileName] stringByAppendingPathExtension:_type];
...
{
    NSString* fileDirectory = [[PNPAudioCommonHelper getCacheDirectory]stringByAppendingPathComponent:_fileName];
    return fileDirectory;
}
...
+ (void)removeRecordedFile:(NSString*)fileName type:(NSString*)type {
    ...
    NSString *path = [PNPAudioCommonHelper getPathByFileName:fileName ofType:type];
